    ...
    <header className={`sticky top-0 z-50 transition-all duration-500 ${
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-24">
          <Link to="/" className="flex items-center space-x-4 transform transition-all duration-500 hover:scale-105 group">
                    ...
                    {item.hasDropdown ? (
                      <div className="flex items-center">
                        <div className={`px-2 m-2 py-4 rounded-2xl text-sm font-semibold cursor-pointer transition-all duration-500 flex items-center space-x-3 relative ${isActive(item.path) ? 'text-green-700 bg-green-100/80 shadow-inner' : 'text-gray-700 hover:text-green-800 hover:bg-white/80'} group-hover:scale-105 group-hover:shadow-2xl`}>
                          ...
                          <span>{item.name}</span>
                          <ChevronDown className={`w-5 h-5 transition-transform duration-500 ${(item.dropdownType === 'explore' && isExploreDropdownOpen) || (item.dropdownType === 'apply' && isApplyDropdownOpen) ? 'rotate-180' : ''}`} />
                            ...
                            <div className="grid grid-cols-2 gap-4 p-6">
                              {loading ? (<div className="col-span-2 text-center py-8"><div className="inline-flex items-center space-x-2 text-green-600"><div className="w-4 h-4 bg-green-400 rounded-full animate-bounce"></div><div className="w-4 h-4 bg-green-400 rounded-full animate-bounce" style={{animationDelay: '0.1s'}}></div><div className="w-4 h-4 bg-green-400 rounded-full animate-bounce" style={{animationDelay: '0.2s'}}></div></div></div>) : (
                                destinations.slice(0, 4).map((dest) => (
                                  <Link key={dest._id} to={`/explore/${dest._id}`} onClick={handleDropdownItemClick} className="group relative bg-gradient-to-br from-white to-green-50 rounded-2xl p-4 hover:shadow-2xl transition-all duration-500 hover:scale-105 border border-green-100/50 overflow-hidden">
                                    ...
                                    <div className="flex items-center space-x-4 relative z-10">
                                      <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 shadow-lg group-hover:shadow-xl transition-shadow duration-300"><img src={dest.thumbnailImage} alt={dest.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /></div>
                                      <div className="flex-1 min-w-0"><h4 className="font-bold text-lg text-gray-800 group-hover:text-green-700 transition-colors duration-300 flex items-center">{dest.name}<span className="ml-2 text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">Popular</span></h4><p className="text-sm text-gray-600 line-clamp-2 mt-1">{dest.description?.substring(0, 70) || 'Explore this beautiful destination...'}</p><div className="flex items-center space-x-3 mt-2"><div className="flex items-center text-xs text-gray-500"><MapPin className="w-3 h-3 mr-1 text-green-500" /><span>{dest.address?.location?.split(',')[0] || 'Jharkhand'}</span></div><div className="flex items-center text-xs text-gray-500"><Star className="w-3 h-3 mr-1 text-yellow-400 fill-current" /><span>{dest.rating || '4.5'}</span></div></div></div>
                                    </div>
                                  </Link>
                                ))
                              )}
                            </div>
                            <div className="border-t border-green-100/50 mt-2 pt-4 px-8"><Link to="/explore" onClick={handleDropdownItemClick} className="group flex items-center justify-center w-full py-3 text-lg font-semibold text-green-600 hover:text-green-800 transition-all duration-300 bg-green-50/50 rounded-xl hover:bg-green-100/80"><span className="group-hover:translate-x-2 transition-transform duration-300">View All Destinations</span><span className="ml-2 group-hover:translate-x-2 transition-transform duration-300">→</span></Link></div>
                          </div>
                        )}
                      ...
                      <Link to={item.path} onClick={handleDropdownItemClick} className={`group relative px-6 py-4 m-0 rounded-2xl text-sm font-semibold transition-all duration-500 flex items-center space-x-3 ${isActive(item.path) ? 'text-green-700 bg-green-100/80 shadow-inner p-2' : 'text-gray-700 hover:text-green-800 hover:bg-white/80'} hover:scale-105 hover:shadow-2xl`}>
                        <IconComponent className="w-5 h-5 transition-transform duration-500 group-hover:scale-110" />
                        <span>{item.name}</span>
                        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-1 bg-gradient-to-r from-green-400 to-green-600 rounded-full transition-all duration-500 group-hover:w-3/4"></div>
                      </Link>
            ...
            <div className="relative group ml-4" ref={profileDropdownRef} onMouseEnter={handleProfileMouseEnter} onMouseLeave={handleProfileMouseLeave}>
              ...
              {isProfileDropdownOpen && (
                <div className="absolute top-full right-0 mt-3 w-48 bg-white/95 backdrop-blur-lg rounded-2xl shadow-2xl border border-green-100/50 py-2 z-20 animate-fade-in-down">
                  {isAuthenticated ? (
                    <button onClick={handleLogout} className="w-full flex items-center space-x-3 p-3 text-sm font-medium text-gray-700 hover:bg-green-50/50 hover:text-green-700 transition-colors duration-200">
                      <LogOut className="w-5 h-5" />
                      <span>Logout</span>
                    </button>
                  ) : (
                    profileOptions.map((option) => { const IconComponent = option.icon; return (<Link key={option.name} to={option.path} onClick={handleDropdownItemClick} className="flex items-center space-x-3 p-3 text-sm font-medium text-gray-700 hover:bg-green-50/50 hover:text-green-700 transition-colors duration-200"><IconComponent className="w-5 h-5" /><span>{option.name}</span></Link>); })
                  )}
                </div>
              )}
            </div>
          </div>
          <button className="lg:hidden p-3 rounded-2xl hover:bg-green-100/50 transition-all duration-300 group" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
            {isMobileMenuOpen ? (<X size={32} className="text-green-700 group-hover:scale-110 transition-transform" />) : (<Menu size={32} className="text-gray-700 group-hover:text-green-700 group-hover:scale-110 transition-transform" />)}
          </button>
        </div>
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-lg shadow-2xl border-t border-green-100 animate-slide-down">
            <div className="p-6 space-y-2">
              {navItems.map((item) => {
                      ...
                      {item.dropdownType === 'apply' && (
                        <div className="ml-4 space-y-1">
                          {applyOptions.map((option) => {
                            ...
                            return (
                              <Link key={option.name} to={option.path} onClick={handleDropdownItemClick} className="flex items-center space-x-3 p-3 rounded-xl text-sm font-medium text-gray-600 hover:text-green-700 hover:bg-green-50/50 transition-all duration-300">
                ...
                return (
                  <Link key={item.name} to={item.path} onClick={handleDropdownItemClick} className={`flex items-center space-x-4 p-4 rounded-2xl text-lg font-semibold transition-all duration-300 hover:scale-105 hover:shadow-lg ${isActive(item.path) ? 'text-green-700 bg-green-100/80' : 'text-gray-700 hover:text-green-800 hover:bg-green-50/50'}`}>
                    <IconComponent className="w-5 h-5" />
                    <span>{item.name}</span>
                  </Link>
                );
              ...
              <div className="space-y-2 border-t pt-4 mt-4 border-green-100/50">
                {isAuthenticated ? (
                    <button onClick={handleLogout} className="w-full flex items-center space-x-4 p-4 rounded-2xl text-lg font-semibold text-gray-700 hover:text-green-800 hover:bg-green-50/50 transition-all duration-300">
                      <LogOut className="w-5 h-5" />
                      <span>Logout</span>
                    </button>
                ) : (
                  profileOptions.map((option) => {
                    const OptionIcon = option.icon;
                    return (
                      <Link key={option.name} to={option.path} onClick={handleDropdownItemClick} className="flex items-center space-x-4 p-4 rounded-2xl text-lg font-semibold text-gray-700 hover:text-green-800 hover:bg-green-50/50 transition-all duration-300">
